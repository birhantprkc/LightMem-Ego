...
def _load_evidence_to_em2mem(monkeypatch):
    _install_lightweight_stubs(monkeypatch)
    spec = importlib.util.spec_from_file_location(
        "online_memory.evidence_to_em2mem",
        ROOT / "online_memory" / "evidence_to_em2mem.py",
    )
    assert spec is not None
    module = importlib.util.module_from_spec(spec)
    assert spec.loader is not None
    monkeypatch.setitem(sys.modules, "online_memory.evidence_to_em2mem", module)
