...
def _default_attn_implementation() -> str:
    requested = (
        os.getenv("EM2MEM_TEXT_EMBED_ATTN_IMPLEMENTATION")
        or os.getenv("EM2MEM_QWEN3_ATTN_IMPLEMENTATION")
        or ""
    ).strip()
    if requested:
        return requested
...
def _resolve_embedding_device(device: str) -> str:
    requested = (
        os.getenv("EM2MEM_TEXT_EMBED_DEVICE")
        or os.getenv("EM2MEM_EMBEDDING_DEVICE")
        or device
        or "auto"
    ).strip()
