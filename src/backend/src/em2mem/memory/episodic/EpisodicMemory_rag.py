# """
# Episodic Memory module for Em2Mem.

# Dense-RAG rewrite of the user's current EpisodicMemory:
# - removes HippoRAG/OpenIE graph construction from the coarse retrieval path
# - precomputes dense document embeddings once for all loaded docs
# - supports query-time filtering by prefix mask (end_time <= until_time)
# - keeps sidecar triplets / sidecar graph rerank, temporal expansion, and entity expansion
# - uses thread-local query-time state so index()+retrieve() can be used safely by parallel worker threads

# Notes:
# - This is "DPR-style" dense retrieval in the engineering sense: dense query/doc embedding + cosine similarity.
# - The actual embedding backbone still comes from the project's EmbeddingModel abstraction.
# - For best efficiency, call build_dense_index() once after loading data and before evaluation.
# """
...
#         self.captions: Dict[str, List[CaptionEntryRAG]] = {g: [] for g in self.granularities}
#         self.caption_id_to_entry: Dict[str, CaptionEntryRAG] = {}
#         self.doc_id_to_entry: Dict[str, Dict[str, CaptionEntryRAG]] = {g: {} for g in self.granularities}
#         self.text_to_entries: Dict[str, Dict[str, List[CaptionEntryRAG]]] = {g: {} for g in self.granularities}

#         self.triplets_by_doc: Dict[str, Dict[str, List[List[str]]]] = {g: {} for g in self.granularities}
#         self.graph_sidecar: Dict[str, Dict[str, GraphEventSidecarRAG]] = {g: {} for g in self.granularities}
...
#     def _get_critical_speech_lines(self, entry: CaptionEntryRAG) -> List[str]:
...
#     def _entry_retrieval_text(self, entry: CaptionEntryRAG) -> str:
#         parts = [entry.text]
...
#     def _entry_embedding_text(self, entry: CaptionEntryRAG) -> str:
...
#             sidecar[doc_id] = GraphEventSidecarRAG(
#                 event_id=event_id,
#                 doc_id=doc_id,
#                 granularity=granularity,
#             )
...
#         indexed_entries: Dict[str, List[CaptionEntryRAG]] = {}
#         for granularity in self.granularities:
...
#     def _lookup_entry_from_index(self, granularity: str, idx: int) -> Optional[CaptionEntryRAG]:
#         entries = self.captions.get(granularity, [])
#         if idx < 0 or idx >= len(entries):
#             return None
#         return entries[idx]

#     def _entry_retrieval_tokens(self, entry: CaptionEntryRAG) -> set:
#         return _tokenize(self._entry_retrieval_text(entry))

#     def _entry_metadata_tokens(self, entry: CaptionEntryRAG) -> set:
#         toks = set()
...
#     ) -> List[Tuple[CaptionEntryRAG, float]]:
#         if not candidates:
#             return []
...
#     def _get_link_entities_for_entry(self, entry: CaptionEntryRAG, granularity: str) -> Set[str]:
#         labels: Set[str] = set()
...
#         self,
#         granularity: str,
#         ranked_candidates: List[Tuple[CaptionEntryRAG, float]],
#     ) -> List[Tuple[CaptionEntryRAG, float]]:
#         if not ranked_candidates:
#             return []

#         indexed_time = self._get_thread_indexed_time()
#         expanded: Dict[str, Tuple[CaptionEntryRAG, float]] = {}
...
#             scored_neighbors: List[Tuple[CaptionEntryRAG, float]] = []
...
#     def expand_entry_to_30s_doc_ids(self, entry: CaptionEntryRAG) -> List[str]:
...
#     def get_caption_by_doc_id(self, doc_id: str, granularity: Optional[str] = None) -> Optional[CaptionEntryRAG]:
...
#     def get_parent_caption(self, doc_id: str, parent_granularity: str = "3min") -> Optional[CaptionEntryRAG]:
...
#         best_parent: Optional[CaptionEntryRAG] = None
...
#     def retrieve_captions_as_str(self, entries: List[CaptionEntryRAG], include_visual_summary: bool = True) -> str:
...
#         all_candidates: List[Tuple[CaptionEntryRAG, float]] = []
...
#             raw_candidates: List[Tuple[CaptionEntryRAG, float]] = []
...
#         if dedup_by_doc_id:
#             best_by_doc: Dict[str, Tuple[CaptionEntryRAG, float]] = {}
...
#     ) -> Union[List[CaptionEntryRAG], str]:
#         if top_k_per_granularity is None:
#             top_k_per_granularity = {"30sec": 10, "3min": 5, "10min": 5, "1h": 3}
...
#     def get_caption_by_id(self, caption_id: str) -> Optional[CaptionEntryRAG]:
...
#     def _project_ranked_candidates_to_30s(
#         self,
#         ranked: List[Tuple[CaptionEntryRAG, float]],
#         max_target_per_entry: int = 16,
#     ) -> Dict[str, float]:
...
#             target_doc_ids = self.expand_entry_to_30s_doc_ids(entry)
#             target_doc_ids = [
#                 doc_id for doc_id in target_doc_ids[:max_target_per_entry]
#                 if self.get_caption_by_doc_id(doc_id, "30sec") is not None
#             ]
...
#         return 0.75 * direct_overlap + 0.25 * metadata_overlap + bonus

#     def _family_graph_refine_30s(
#         self,
#         query: str,
...
#         final_items: List[Tuple[CaptionEntryRAG, float]] = []
...
#         if not ranked:
#             return []
#         projected = self._project_ranked_candidates_to_30s(ranked)
#         refined = self._family_graph_refine_30s(query, family_info, projected)
#         if refined:
#             return refined
...
#     def _projection_local_score_30s(self, query_tokens: Set[str], doc_id: str) -> float:
#         entry = self.get_caption_by_doc_id(doc_id, "30sec")
#         if entry is None:
#             return 0.0
...
#             base = float(score) * rank_bonus * self._granularity_projection_weight(entry.granularity)
#             target_doc_ids = []
#             for doc_id in self.expand_entry_to_30s_doc_ids(entry):
#                 child_entry = self.get_caption_by_doc_id(doc_id, "30sec")
#                 if child_entry is None:
#                     continue
...
#                     local_score = self._projection_local_score_30s(query_tokens, doc_id)
...
#         if not ranked:
#             return []
#         projected = self._project_ranked_candidates_to_30s(
#             query=query,
#             ranked=ranked,
...
class EpisodicMemoryRAG:
    ...
    def normalize_doc_ids_to_30s(self, doc_ids: List[str]) -> List[str]:
        normalized: List[str] = []
        seen: Set[str] = set()
        for raw_doc_id in doc_ids or []:
            ...
            if entry is None:
                continue
            for child_id in self.expand_entry_to_30s_doc_ids(entry):
    ...
    def expand_seed_doc_ids_with_neighbors(
        ...
        seeds = self.normalize_doc_ids_to_30s(seed_doc_ids)
        expanded: List[str] = []
        seen: Set[str] = set()
    ...
    def retrieve_ranked_from_doc_id_pool(
        ...
        indexed_time = self._get_thread_indexed_time()
        query_tokens = _tokenize(query)
        seed_doc_ids = self.normalize_doc_ids_to_30s(doc_ids)
        ...
        results: List[Tuple[CaptionEntryRAG, float]] = []
