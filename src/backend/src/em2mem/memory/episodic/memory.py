...
# - exposes ranked multiscale candidates so EM2Memory can project them to 30s anchors
# """
...
class EpisodicMemory:
    ...
    def _load_cached_hipporag_for_retrieval(self, hipporag: HippoRAG, expected_docs: int, granularity: str) -> bool:
        ...
        """
        cache_only = str(os.getenv("EM2MEM_QUERY_USE_CACHED_HIPPORAG", "1")).lower() not in {"0", "false", "no"}
        if not cache_only:
            return False
        strict_load_only = str(os.getenv("EM2MEM_QUERY_STRICT_LOAD_ONLY", "1")).lower() not in {"0", "false", "no"}
        try:
