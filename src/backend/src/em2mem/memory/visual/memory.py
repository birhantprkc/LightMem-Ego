# """
# Visual Memory module for Em2Mem.
# """
...
# Lightweight event-grounded version:
# - keeps optional embedding-based clip retrieval for compatibility
# - primarily serves as an event visual evidence store
# - directly returns keyframes for final selected event anchors
# """
...
#     - load 30s evidence units containing doc_id / keyframe_paths / video_path
...
class VisualMemory:
    ...
    def set_base_dir(self, base_dir: str | os.PathLike[str] | None) -> None:
        self.base_dir = os.fspath(base_dir) if base_dir else None
    ...
    def get_event_images(
        ...
        for doc_id in doc_ids:
            ...
            if entry.keyframe_paths:
                for img_path in entry.keyframe_paths[:max_images_per_event]:
                    resolved_img_path = self._resolve_path(img_path)
                    if not os.path.exists(resolved_img_path):
                        logger.warning(f"Keyframe file not found: {resolved_img_path}")
                        continue
                    try:
                        img = Image.open(resolved_img_path).convert("RGB")
                        img.filename = resolved_img_path
                        images.append(img)
                    except Exception as e:
                        logger.warning(f"Failed to load keyframe {resolved_img_path}: {e}")

            # fallback to video extraction
            if not images and entry.video_path:
                frames = self._extract_frames(
                    self._resolve_path(entry.video_path),
                    fps=1.0,
                    max_frames=max_images_per_event,
                )
                images = [f.frame for f in frames if f.frame is not None]
